...
tote_asset = gym.load_asset(sim, asset_root, "urdf/carter/carter.urdf", tote_opts)
...
for i in range(len(dof_props)):
    ...
    dof_props['damping'][i] = 600.0  # 미끄러짐 방지 댐핑
gym.set_actor_dof_properties(env, tote_handle, dof_props)

wheel_velocities = np.zeros(4, dtype=np.float32)
wheel_radius = 0.14
wheel_base = 0.58

# =================================================================
# [간단 제어] 6. 앞뒤 2m 이동을 위한 변수 설정
# =================================================================
start_pos = None          # 이동 시작 지점을 저장할 변수
start_yaw = None          # 회전 시작 방향을 저장할 변수
state = "DRIVE"           # 현재 로봇 상태: "DRIVE" (직진) 또는 "TURN" (회전)
distance_target = 2.0     # 목표 이동 거리 (2미터)
# =================================================================
...
while not gym.query_viewer_has_closed(viewer):
    ...
    # [수정] 액터의 상태 데이터를 배열로 가져옵니다.
    body_states = gym.get_actor_rigid_body_states(env, tote_handle, gymapi.STATE_POS)
    
    # 카터 로봇의 루트 링크(chassis)는 0번 인덱스입니다.
    # 포즈(pose) 정보에서 위치(p)와 회전(r)을 추출합니다.
    robot_pose = body_states['pose'][0] 
    
    # 현재 X, Y 위치 추출
    current_pos = np.array([robot_pose['p']['x'], robot_pose['p']['y']])
    
    # 쿼터니언(w, x, y, z)을 활용해 현재 로봇의 Yaw(헤딩 각도) 계산
    qw = robot_pose['r']['w']
    qx = robot_pose['r']['x']
    qy = robot_pose['r']['y']
    qz = robot_pose['r']['z']
    
    current_yaw = math.atan2(2 * (qw * qz + qx * qy), 1 - 2 * (qy * qy + qz * qz))
    ...
    # --- 상태 머신 제어 로직 ---
    if state == "DRIVE":
        # 기준점이 지정되지 않았다면 현재 위치를 시작점으로 기록
        ...
        if moved_distance < distance_target:
            linear_vel = 0.6  # 0.6 m/s 속도로 직진
        else:
            # 2m 이동을 완료하면 회전 상태로 전환
            linear_vel = 0.0
            start_pos = None
            state = "TURN"
            
    elif state == "TURN":
        # 회전 기준 방향이 없다면 현재 방향을 시작 방향으로 기록
        if start_yaw is None:
            start_yaw = current_yaw
            print("[상태] 제자리 180도 회전 시작")
            
        # 시작 방향으로부터 회전한 각도 오차 계산 (정규화)
        angle_turned = current_yaw - start_yaw
        angle_turned = math.atan2(math.sin(angle_turned), math.cos(angle_turned))
        
        # 180도(pi 라디안)에 도달할 때까지 회전 오차 측정
        # 반대 방향인 pi(3.14159)와의 차이를 계산
        error_to_180 = math.pi - abs(angle_turned)
        
        if error_to_180 > 0.05:  # 오차가 약 3도 이내로 들어올 때까지 회전
            angular_vel = 1.0   # 1.0 rad/s 속도로 제자리 회전
        else:
            # 회전이 끝나면 다시 직진 상태로 전환
            angular_vel = 0.0
            start_yaw = None
            state = "DRIVE"
    ...
    wheel_velocities[0] = left_wheel_vel  # fl
    wheel_velocities[1] = right_wheel_vel # fr
    wheel_velocities[2] = left_wheel_vel  # rl
    wheel_velocities[3] = right_wheel_vel # rr
