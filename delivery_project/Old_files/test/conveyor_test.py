...
fixed_caser_handle = gym.create_actor(env, fixed_caser_asset, gymapi.Transform(p=gymapi.Vec3(0.0, 0.0, 3.0)), "fixed_caser_asset", -1, 0)

taping_asset = gym.load_asset(sim, asset_root, "urdf/taping/v1/taping_v1.urdf", machinery_opts)
taping_handle = gym.create_actor(env, taping_asset, gymapi.Transform(p=gymapi.Vec3(0.0, 0.0, 5.0)), "taping_asset", -1, 0)
...
box_manager = CardboardBoxManager(gym, sim, env, asset_root, pose=gymapi.Transform(p=gymapi.Vec3(0.0, 0.0, 1.5)))
...
gym.viewer_camera_look_at(viewer, env, gymapi.Vec3(-0.82, -0.74, 2.28), gymapi.Vec3(0.85, 1.17, 0.69))
...
gym.subscribe_viewer_keyboard_event(viewer, gymapi.KEY_3, "toggle_auto_rotation") # 3번 키
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if is_landed and (-1.8 <= relative_y <= 1.8) and not is_grabed:
        ...
        # 🛠️ [추가] 상자의 회전축 강제 강제 정렬 (자세 초기화 트릭)
        # 현재 상자의 위치 좌표(p)는 유지하고, 회전(r)만 완벽한 수평 상태로 고정합니다.
        current_transform = gym.get_rigid_transform(env, global_box_body_index)
        
        # 쿼터니언 (0, 0, 0, 1)은 회전이 전혀 없는 순수 정자세를 뜻합니다.
        current_transform.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0) 
        
        # 강제로 정자세 덮어쓰기
        gym.set_rigid_transform(env, global_box_body_index, current_transform)
