...
pallet = gymapi.AssetOptions()
pallet.fix_base_link = False
pallet_asset = gym.load_asset(sim, asset_root, "urdf/pallet/pallet_test.urdf", pallet)

door = gymapi.AssetOptions()
door.fix_base_link = True
door_asset = gym.load_asset(sim, asset_root, "urdf/door/door.urdf", door)
...
# pallet_handle = gym.create_actor(env, pallet_asset, gymapi.Transform(p=gymapi.Vec3(0.0, 0.0, 2)), "pallet_test_actor", -1, 0)

pitch_quat = gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 0, 0), math.radians(90.0))
yaw_quat = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), math.radians(180.0))
combined_quat = yaw_quat * pitch_quat
door_handle = gym.create_actor(env, door_asset, gymapi.Transform(p=gymapi.Vec3(-1, -half_l+0.05, 0), r=combined_quat), "door_actor", 0, 0)

# =================================================================
# 🛠️ 로봇 인스턴스 스폰 (중앙 하이웨이 배치)
# =================================================================
franka_handle = gym.create_actor(env, franka_asset, gymapi.Transform(p=gymapi.Vec3(-1.0, 0.0, 0.055)), "franka", -1, 0)
...
spawn_quat = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), np.pi / 2.0)
carter_transform = gymapi.Transform(
    p=gymapi.Vec3(0, 0.7, 0.30),
    r=spawn_quat
)
carter_handle = gym.create_actor(env, carter_asset, carter_transform, "carter", -1, 0)
# carter_handle = gym.create_actor(env, carter_asset, gymapi.Transform(p=gymapi.Vec3(0, 0.7, 0.30)), "carter", -1, 0)

tray_handle = gym.create_actor(env, tray_asset, gymapi.Transform(p=gymapi.Vec3(0, 0.7, 0.75)), "tray", -1, 0)

low_amr_handle = gym.create_actor(env, low_amr_asset, gymapi.Transform(p=gymapi.Vec3(1.5, 0.77, 0.20)), "low_amr", -1, 0)
...
# =================================================================
print("\n[시스템] 선반을 양쪽 벽으로 밀착 배치하여 중앙 로봇 주행로를 확보합니다...")
...
print(carter_planner.resolution," ",low_planner.resolution)
...
print("amr_ctrl pos ", initial_c_states['pose']['p'][0]['x'], " ", initial_c_states['pose']['p'][0]['y'])

start_pos_coord = np.array([1.5, 0.77])
