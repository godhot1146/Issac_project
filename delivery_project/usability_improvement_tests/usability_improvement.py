...
from raycast_picker import RaycastPicker
...
# ============================================================================
# [SECTION 6-1] 레이캐스트 피킹 대상 등록
# ============================================================================
picker = RaycastPicker(gym, sim, viewer, env)

picker.register_obb(w_back, "wall_back", half_extent=(wall_thickness / 2, room_y / 2, wall_height / 2))
picker.register_obb(w_front, "wall_front", half_extent=(wall_thickness / 2, room_y / 2, wall_height / 2))
picker.register_obb(w_right, "wall_right", half_extent=(room_x / 2, wall_thickness / 2, wall_height / 2))
picker.register_obb(w_left, "wall_left", half_extent=(room_x / 2, wall_thickness / 2, wall_height / 2))
# half_extent를 넣으면 marker_radius는 자동으로 박스를 감싸는 외접구 크기로 계산됩니다.
# 필요하면 register_obb(..., marker_radius=3.0)처럼 직접 지정해서 더 크게/작게 조절 가능합니다.

picker.register_sphere(indy7_arm_1.handle, "indy7_arm_1", radius=0.6, marker_radius=0.8)
picker.register_sphere(indy7_arm_2.handle, "indy7_arm_2", radius=0.6, marker_radius=0.8)
picker.register_sphere(indy7_arm_3.handle, "indy7_arm_3", radius=0.6, marker_radius=0.8)

picker.register_sphere(low_handle, "low_amr", radius=0.4, marker_radius=0.55)
picker.register_sphere(forklift_handle, "forklift", radius=0.6, marker_radius=0.8)

for i, box_manager in enumerate(box_managers, start=1):
    picker.register_sphere(box_manager.handle, f"cardboard_box_{i}", radius=0.2, marker_radius=0.3)
